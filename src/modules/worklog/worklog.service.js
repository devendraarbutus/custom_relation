...
const autoStopPreviousTimer = async (employeeId) => {
  const logs = await WorkLog.find({
    employeeId,
    timer: { $elemMatch: { endTime: null } }
  });
  ...
  for (const log of logs) {
    ...
    if (running) {
      ...
      log.lastStopTime = running.endTime;
      await log.save();
    }
  }
};
...
const startTimer = async (employeeId, taskId) => {
  ...
  await autoStopPreviousTimer(employeeId);
  ...
  if (log.timer.some((t) => !t.endTime)) {
    return { success: false, message: "Timer already running on this task" };
  }
